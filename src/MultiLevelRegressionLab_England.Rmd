...
```{r, echo=FALSE, out.width="50%", out.height="50%", fig.cap="DAG", fig.a}
...
$$
\bigl[ \boldsymbol{\alpha},\beta,\sigma^2 \mid \boldsymbol{y} \bigr] \propto \prod_{i=1}^{n} \prod_{j=1}^{J}  \bigl[ \log(y_{ij})\mid g\big(\alpha_{j},\beta,\log(x_{ij})\big),\sigma^{2} \bigr] \bigl[\alpha_{j} \bigr] \bigl[ \beta \bigr] \bigl[ \sigma  \bigr]
$$
...
$$
\bigl[ \alpha_{j},\beta, \mu_{\alpha}, \sigma^2, \varsigma_{\alpha}^2  \mid \boldsymbol{y} \bigr] \propto \prod_{i=1}^{n} \prod_{j=1}^{J}  \bigl[ \log(y_{ij})\mid g\big(\alpha_{j}, \beta,\log(x_{ij})\big),\sigma^{2} \bigr] \bigl[\alpha_{j} \mid \mu_{\alpha}, \varsigma_{\alpha}^2 \bigr] \bigl[ \beta \bigr] \bigl[ \sigma  \bigr] \bigl[ \varsigma  \bigr]
$$
...
##################################################################
{ # Extra bracket needed only for R markdown files - see answers
  sink("NO2JAGS_randomints.R") # This is the file name for the jags code
  cat("
  model{
    # priors
    beta ~ dnorm(0,1E-6)
...
# summary
MCMCvis::MCMCsummary(zc_randomints, params = c("beta", "sigma", "mu_alpha", "varsigma"))
