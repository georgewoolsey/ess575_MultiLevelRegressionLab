...
\textcolor{violet}{The hypothesis represented by this model is that relationship between emissions and fertilizer application rate is the same for all sites and fertilizer types. That is, we are not allowing the relationship between emissions and fertilizer application rate to vary across sites or fertilizer types.}
...
\textcolor{violet}{There is evidence of sufficient convergence of the $\beta$ and $\sigma$ parameters. The values for Rhat near 1 in the summary table suggest convergence } ([Brooks and Gelman 1998](https://www2.stat.duke.edu/~scs/Courses/Stat376/Papers/ConvergeDiagnostics/BrooksGelman.pdf)).

Caterpillar plot for parameters.
...
Make a horizontal caterpillar plot for the $\boldsymbol{\alpha}$.
...
\textcolor{violet}{There is evidence of sufficient convergence of the $\beta$, $\sigma$, $\mu_{\alpha}$, and $\varsigma_{\alpha}$ parameters. The values for Rhat near 1 in the summary table suggest convergence } ([Brooks and Gelman 1998](https://www2.stat.duke.edu/~scs/Courses/Stat376/Papers/ConvergeDiagnostics/BrooksGelman.pdf)).
...
\textcolor{violet}{Compared to the predictions of the pooled model shown above (*Visualizing the pooled model predictions*), the predicted log emissions from the random intercept model have much greater uncertainty as shown by the wide confidence bands. The random intercept model treats the site intercepts as partially pooled wheras the pooled model completely ignored site-level effects on the intercepts.}

#### Site-level model predictions
...
\textcolor{violet}{The intercepts differ for some sites between the no-pool model and the random-intercepts model due to borrowing strength. Hierarchical modelling is able to produce estimates of the intercepts from sites with few observations by integrating information from sites with larger data sets (Hobbs and Hooten 2015). In this example, sites with fewer observations (e.g. site #51) have an estimated intercept that is closer to the overall mean while sites with more observations and less variance (e.g. site #56) have an estimated intercept that is less influenced by the overall mean.}

[Hobbs and Hooten, 2015](https://www.jstor.org/stable/j.ctt1dr36kz)
...
\textcolor{violet}{There is evidence of sufficient convergence of the $\beta$, $\sigma$, $\kappa$, $\eta$, and $\varsigma_{\alpha}$ parameters. The values for Rhat near 1 in the summary table suggest convergence } ([Brooks and Gelman 1998](https://www2.stat.duke.edu/~scs/Courses/Stat376/Papers/ConvergeDiagnostics/BrooksGelman.pdf)).
...
```{r, fig.cap="Overall - random intercepts, group-level effect model", fig.height=4, fig.width=6}
g7 <- g1 +
  geom_line(
...
```{r, fig.cap="Overall - random intercepts, No group-level effect model", fig.height=4, fig.width=6}
gridExtra::grid.arrange(g5, g6, nrow = 1)
```
...
\textcolor{violet}{Compared to the random effects model, modeling the intercept using site soil carbon does not have much affect on the uncertainty in predicting emissions. This means that allowing the intercept to vary across site based on soil carbon does not result in a significant change in the predicted site-level intercept versus the random effects model which modeled site-level intercepts without being informed by soil carbon.}
...
```{r}
n.sites <- length(unique(BayesNSF::N2OEmission$group.index))
n.ferts <- length(unique(BayesNSF::N2OEmission$fert.index))
...
data = list(
  ...
  #divide by 100 to make data a proportion, take logit, and center
  w = boot::logit(BayesNSF::SiteCarbon$mean/100) - mean(boot::logit(BayesNSF::SiteCarbon$mean/100)),
  fertilizer = BayesNSF::N2OEmission$fert.index, 
  group = BayesNSF::N2OEmission$group.index,
  n.sites = n.sites,
  n.ferts = n.ferts)
...
##################################################################
{ # Extra bracket needed only for R markdown files - see answers
  sink("NO2JAGS_randomcoeffs.R") # This is the file name for the jags code
  cat("
  model{
    ## priors
      # y priors
...
stats::update(jm, n.iter = n.update)
# save the coda object (more precisely, an mcmc.list object) to R as "zc"
zc_randomcoeffs = rjags::coda.samples(
  model = jm
  , variable.names = c("alpha", "beta", "sigma", "kappa", "eta", "varsigma_alpha", "mu_beta", "varsigma_beta")
  , n.iter = n.iter
  , n.thin = 1
)
```
...
#####################
# trace plot
MCMCvis::MCMCtrace(zc_randomcoeffs, params = c(
    "sigma"
    , "kappa"
    , "eta"
    , "mu_beta"
...
# summary
MCMCvis::MCMCsummary(zc_randomcoeffs, params = c(
    "sigma"
    , "kappa"
    , "eta"
    , "mu_beta"
...
\textcolor{violet}{There is evidence of sufficient convergence of the $\sigma$, $\kappa$, $\eta$, $\mu_{\beta}$, $\varsigma_{\beta}$, and $\varsigma_{\alpha}$ parameters. The values for Rhat near 1 in the summary table suggest convergence } ([Brooks and Gelman 1998](https://www2.stat.duke.edu/~scs/Courses/Stat376/Papers/ConvergeDiagnostics/BrooksGelman.pdf)).

Caterpillar plot for parameters.

```{r}
# Caterpillar plots
MCMCvis::MCMCplot(zc_randomcoeffs, params = c(
    "sigma"
    , "kappa"
    , "eta"
    , "mu_beta"
...
\textcolor{violet}{A caterpillar plot for the $\boldsymbol{\beta}$ would allow us to assess whether fertilizer type a good predictor of emission and compare the slope across fertilizer type. Based on the caterpiller plot above, there is not much variation in the slope based on fertilizer type when considering the confidence interval of each $\beta_k$.}
...
knitr::include_graphics("../data/eq1.jpg")
```
...
```{r, echo=FALSE, out.width="55%", out.height="55%", fig.cap="", fig.align='center'}
knitr::include_graphics("../data/eq2.jpg")
```

The covariance terms are defined as $\text{Cov}\bigl( \boldsymbol{\alpha}, \boldsymbol{\beta} \bigr) = \rho \varsigma_{\alpha} \varsigma_{\beta}$ where $\rho$ is the coefficient of correlation between $\boldsymbol{\alpha}$ and $\boldsymbol{\beta}$.

### Question 5

Draw a Bayesian network and write out the posterior and joint distribution for a linear regression model of $\textrm{N} _2 \textrm{O}$ emission ($y_{ij}$) on fertilizer addition ($x_{ij}$).

```{r, echo=FALSE, out.width="50%", out.height="50%", fig.cap="DAG", fig.align='center'}
knitr::include_graphics("../data/DAG6.jpg")
```


### Question 6

Write out the posterior and joint distributions for a linear regression model of $\textrm{N} _2 \textrm{O}$ emission ($y_{ij}$) on fertilizer addition ($x_{ij}$). Choose appropriate distributions for each random variable.

* Remember: *$\rho$ = coefficient of correlation = $-1 \le \rho \le 1$*

```{r, echo=FALSE, out.width="50%", out.height="50%", fig.cap="", fig.align='center'}
knitr::include_graphics("../data/eq2.jpg")
```

$$
\begin{aligned}
g \bigl(\alpha_{j},\beta_{k},\log(x_{ij}) \bigr) = \alpha_{j} + \beta_{k} \bigl(\log(x_ij) \bigr)\\
\end{aligned}
$$
...
$$
\begin{aligned}
\bigl[ \alpha_{j},\beta_{j}, \mu_{\alpha}, \mu_{\beta}, \sigma^2, \varsigma_{\alpha}^2, \varsigma_{\beta}^2  \mid \boldsymbol{y} \bigr] \propto \prod_{i=1}^{n}  \prod_{j=1}^{J} {\sf normal} \bigr( \log(y_{ij}) \mid g \bigl( \alpha_{j}, \beta_{j}, \log(x_{ij})  \bigr), \sigma^{2}\bigr)\\
\times \; {\sf multivariate \ normal} \biggr(\begin{pmatrix} \alpha_{j}\\ \beta_{j} \end{pmatrix} \bigg| \begin{pmatrix} \mu_{\alpha}\\ \mu_{\beta} \end{pmatrix}, \Sigma \biggr) \\ 
\times \; {\sf normal} \bigr(\mu_{\alpha} \mid 0,10000\bigr) \\
\times \; {\sf normal} \bigr(\mu_{\beta} \mid 0,10000\bigr) \\
\times \; {\sf uniform}\bigr(\sigma \mid 0, 100 \bigl) \\
\times \; {\sf uniform}\bigr(\varsigma_{\alpha} \mid 0, 100 \bigl) \\
\times \; {\sf uniform}\bigr(\varsigma_{\beta} \mid 0, 100 \bigl) \\ 
\times \; {\sf uniform}\bigr(\rho \mid 0, 1 \bigl)
\end{aligned}
$$
...
inits = list(
  list(B = B, sigma = 50, mu_alpha = 0, mu_beta = 1.5, varsigma_alpha = 10, varsigma_beta = 10, rho = -.5),
  list(B = B*.5, sigma = 20, mu_alpha = -.2, mu_beta = .8, varsigma_alpha = 50, varsigma_beta = 50, rho = .5),
  list(B = B*.3, sigma = 10, mu_alpha = -.4, mu_beta = 1.8, varsigma_alpha = 30, varsigma_beta = 20, rho = 0))
```
...
##################################################################
{ # Extra bracket needed only for R markdown files - see answers
  sink("NO2JAGS_randomcoeffs_fnl.R") # This is the file name for the jags code
  cat("
  model{
    ## priors
      # coefficient of correlation for covariance matrix
...
######################
jm = rjags::jags.model(
  file = "NO2JAGS_randomcoeffs_fnl.R"
  , data = data
  , inits = inits
  , n.chains = length(inits)
  , n.adapt = n.adapt
...
stats::update(jm, n.iter = n.update)
# save the coda object (more precisely, an mcmc.list object) to R as "zc"
zc_randomcoeffs_fnl = rjags::coda.samples(
  model = jm
  , variable.names = c(
    "rho"
    , "sigma"
...
#####################
# trace plot
MCMCvis::MCMCtrace(zc_randomcoeffs_fnl, params = c(
    "rho"
    , "sigma"
    , 'mu_alpha'
    , "mu_beta"
...
# summary
MCMCvis::MCMCsummary(zc_randomcoeffs_fnl, params = c(
    "rho"
    , "sigma"
    , 'mu_alpha'
    , "mu_beta"
...
\textcolor{violet}{There is evidence of sufficient convergence of the $\rho$, $\sigma$, $\mu_{\alpha}$, $\mu_{\beta}$, $\varsigma_{\alpha}$, and $\varsigma_{\beta}$ parameters. The values for Rhat near 1 in the summary table suggest convergence } ([Brooks and Gelman 1998](https://www2.stat.duke.edu/~scs/Courses/Stat376/Papers/ConvergeDiagnostics/BrooksGelman.pdf)).

Caterpillar plot for parameters.

```{r}
# Caterpillar plots
MCMCvis::MCMCplot(zc_randomcoeffs_fnl, params = c(
    "rho"
    , "sigma"
    , 'mu_alpha'
    , "mu_beta"
...
Make a horizontal caterpillar plot for the $\boldsymbol{\beta}$.
...
where $\mu_i$ is the $i^{th}$ prediction of your model. Overlay the posterior distribution of the simulated data on the histogram of the real data (density on y axis, not frequency). What do you conclude? Is there any indication of lack of fit? Enough to discard the model?
...
##################################################################
{ # Extra bracket needed only for R markdown files - see answers
  sink("NO2JAGS_pooled_check.R") # This is the file name for the jags code
  cat("
  model{
...
stats::update(jm, n.iter = n.update)
# save the coda object (more precisely, an mcmc.list object) to R as "zc"
zc_pooled_check = rjags::coda.samples(
  model = jm
  , variable.names = c(
      #priors
...
```{r}
# summary
MCMCvis::MCMCsummary(zc_pooled_check, params = c(
      # test statistics
      "mean_y"
      , "mean_y_sim"
      , "min_y"
...
### Posterior predictive check - Simulated vs. Observed data

Overlay the posterior distribution of the simulated data on the histogram of the real data (density on y axis, not frequency). What do you conclude? Is there any indication of lack of fit? Enough to discard the model?

```{r}
# extract data
dta_temp <- MCMCvis::MCMCchains(zc_pooled_check, params = c("log.emission_sim")) %>% 
  c() %>% # convert matrix to vector
  as.data.frame() %>%
  dplyr::rename(log.emission_sim = 1)
# temp_hdi <- HDInterval::hdi(dta_temp$diff_x10_x20, credMass = 0.95)

# posterior distribution of the simulated data vs. the real data
  # plot
  ggplot() +
  geom_histogram(
    data = BayesNSF::N2OEmission %>% dplyr::mutate(log.emission = as.double(log(emission)))
    , mapping = aes(x = log.emission,y = ..density.., fill = "observed data")
    , bins = 50
    , alpha = 0.8
  ) +
  geom_density(
    data = dta_temp
    , mapping = aes(x = log.emission_sim, y = ..density.., color = "simulated data")
    , linetype = 2
    , lwd = 1.2
  ) +
  ...
  xlab(latex2exp::TeX("$\\log(\\textrm{N} _2 \\textrm{O}$ emissions)")) +
  ylab("Density") +
  labs(
    title = "Posterior distribution of simulated data versus observed data"
    , subtitle = latex2exp::TeX("$\\log(\\textrm{N} _2 \\textrm{O}$ emissions) - Pooled Model")
  ) +
  theme_bw() + 
  theme(
    legend.position = "top"
...
\textcolor{violet}{Based on the distribution of the observed data compared to the distribution of the simulated data, there is not a significant indication of lack of fit. The largest discrepancy between the observed data and the simulated data is at the low end of $\textrm{N} _2 \textrm{O}$ emissions.}

### Additional posterior predicitve check figures

#### Posterior predictive check - mean plot

```{r}
# PLOT
MCMCvis::MCMCchains(zc_pooled_check, params = c("mean_y", "mean_y_sim")) %>%
  data.frame() %>%
ggplot(data = .) +
  ...
  scale_y_continuous(
    limits = c(
      mean(data$log.emission)-(sd(data$log.emission)*1)
      , mean(data$log.emission)+(sd(data$log.emission)*1)
    ) 
  ...
  xlab(latex2exp::TeX("Mean Observed $\\log(\\textrm{N} _2 \\textrm{O}$ emissions)")) +
  ylab(latex2exp::TeX("Mean Simulated $\\log(\\textrm{N} _2 \\textrm{O}$ emissions)")) +
  labs(
    title = latex2exp::TeX("Observed vs. Simulated $\\log(\\textrm{N} _2 \\textrm{O}$ emissions) - Mean")
    , subtitle = paste0(
        "*Bayesian P value = "
        , MCMCvis::MCMCchains(zc_pooled_check, params = c("p_val_mean")) %>% mean() %>% scales::comma(accuracy = 0.01)
      )
  ) +
  theme_bw()
...
\textcolor{violet}{The Bayesian p-value of `r MCMCvis::MCMCchains(zc_pooled_check, params = c("p_val_mean")) %>% mean() %>% scales::comma(accuracy = 0.01)` indicates good fit for the $\textrm{N} _2 \textrm{O}$ emissions. If the p-value were very large or very small (i.e., close to 1 or 0), it would indicate lack of fit.}

#### Posterior predictive check - SD plot

```{r}
# PLOT
MCMCvis::MCMCchains(zc_pooled_check, params = c("sd_y", "sd_y_sim")) %>%
  data.frame() %>%
ggplot(data = .) +
  geom_abline(intercept = 0, slope = 1, lwd = 1, color = "gray20") +
  geom_point(
  ...
  xlab(latex2exp::TeX("St. Dev. Observed $\\log(\\textrm{N} _2 \\textrm{O}$ emissions)")) +
  ylab(latex2exp::TeX("St. Dev. Simulated $\\log(\\textrm{N} _2 \\textrm{O}$ emissions)")) +
  labs(
    title = latex2exp::TeX("Observed vs. Simulated $\\log(\\textrm{N} _2 \\textrm{O}$ emissions) - St. Dev.")
    , subtitle = paste0(
        "*Bayesian P value = "
        , MCMCvis::MCMCchains(zc_pooled_check, params = c("p_val_sd")) %>% mean() %>% scales::comma(accuracy = 0.01)
      )
  ) +
  theme_bw()
...
\textcolor{violet}{The Bayesian p-value of `r MCMCvis::MCMCchains(zc_pooled_check, params = c("p_val_sd")) %>% mean() %>% scales::comma(accuracy = 0.01)` indicates good fit for the standard deviation of $\textrm{N} _2 \textrm{O}$ emissions. If the p-value were very large or very small (i.e., close to 1 or 0), it would indicate lack of fit.}

#### Posterior predictive check - Minimum plot

```{r}
# PLOT
MCMCvis::MCMCchains(zc_pooled_check, params = c("min_y", "min_y_sim")) %>%
  data.frame() %>%
ggplot(data = .) +
  geom_abline(intercept = 0, slope = 1, lwd = 1, color = "gray20") +
  geom_point(
    mapping  = aes(x = min_y, y = min_y_sim)
    , color = "firebrick"
    , alpha = 0.2
  ) +
  xlab(latex2exp::TeX("Minimum Observed $\\log(\\textrm{N} _2 \\textrm{O}$ emissions)")) +
  ylab(latex2exp::TeX("Minimum Simulated $\\log(\\textrm{N} _2 \\textrm{O}$ emissions)")) +
  labs(
    title = latex2exp::TeX("Observed vs. Simulated $\\log(\\textrm{N} _2 \\textrm{O}$ emissions) - Minimum")
    , subtitle = paste0(
        "*Bayesian P value = "
        , MCMCvis::MCMCchains(zc_pooled_check, params = c("p_val_min")) %>% mean() %>% scales::comma(accuracy = 0.01)
      )
  ) +
  theme_bw()
...
\textcolor{violet}{The Bayesian p-value of `r MCMCvis::MCMCchains(zc_pooled_check, params = c("p_val_min")) %>% mean() %>% scales::comma(accuracy = 0.01)` indicates poor fit for the minimum of $\textrm{N} _2 \textrm{O}$ emissions.}

#### Posterior predictive check - Sum of Squared Errors

```{r}
# PLOT
MCMCvis::MCMCchains(zc_pooled_check, params = c("fit_y", "fit_y_sim")) %>%
  data.frame() %>%
ggplot(data = .) +
  geom_abline(intercept = 0, slope = 1, lwd = 1, color = "gray20") +
  geom_point(
  ...
  xlab(latex2exp::TeX("SSE Observed $\\log(\\textrm{N} _2 \\textrm{O}$ emissions)")) +
  ylab(latex2exp::TeX("SSE Simulated $\\log(\\textrm{N} _2 \\textrm{O}$ emissions)")) +
  labs(
    title = latex2exp::TeX("Observed vs. Simulated $\\log(\\textrm{N} _2 \\textrm{O}$ emissions) - Sum of Squared Errors")
    , subtitle = paste0(
        "*Bayesian P value = "
        , MCMCvis::MCMCchains(zc_pooled_check, params = c("p_val_fit")) %>% mean() %>% scales::comma(accuracy = 0.01)
      )
  ) +
  theme_bw()
...
\textcolor{violet}{The Bayesian p-value of `r MCMCvis::MCMCchains(zc_pooled_check, params = c("p_val_fit")) %>% mean() %>% scales::comma(accuracy = 0.01)` indicates good fit for the discrepancy statistic of $\textrm{N} _2 \textrm{O}$ emissions.
